...
public:
    explicit CoordinateOperator(const CoordinateOperatorParameters &ep_) : ep(ep_)
    {
        if(ep.initial_coordinates.size() > 0)
            raw_coordinates = odesolver::DevDatC(ep.initial_coordinates);
    }

    // Coordinates
    void set_raw_coordinates(const odesolver::DevDatC coordinates);
    ...
    odesolver::DevDatC get_raw_coordinates() const;
    odesolver::DevDatC get_raw_velocities() const;
...
private:
    ...
    odesolver::DevDatC raw_coordinates;
    odesolver::DevDatC raw_velocities;
