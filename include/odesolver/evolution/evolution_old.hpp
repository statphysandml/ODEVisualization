...
// integrators exist, variables should already be complete for explicit implementation (i.e., the parameter class has been adapated
...
public:
     ...
     *     use integrate_adapative - with an additional while loop - the total number of observations depends on the used step_size type and end_t,
...
private:
    ...
    void evolve_(StepperType stepper, odesolver::boost_integrators::Integrator<StepperType, Observer> * integrator, odesolver::DevDatC &initial_coordinates, cudaT start_t=0.0, cudaT end_t=1.0, cudaT delta_t=0.01)
    {
        if(observer and observe_every_nth_step > 1)
        {
            ...
            {
                // ToDo: Check if this is possible with controlled steppers, i.e. to have a variable delta t
