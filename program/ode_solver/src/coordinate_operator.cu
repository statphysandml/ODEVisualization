...
std::vector< Eigen::MatrixXd* > compute_jacobian_elements(const DevDatC &coordinates, JacobianWrapper * jacobian_equations)
{
    ...
    {
        DevDatC jacobian_element_wrapper(1, number_of_coordinates);
...
void CoordinateOperator::set_raw_coordinates(const DevDatC coordinates)
{
    raw_coordinates = coordinates;
}

DevDatC CoordinateOperator::get_raw_coordinates() const
{
    return raw_coordinates;
}

DevDatC CoordinateOperator::get_raw_velocities() const
{
