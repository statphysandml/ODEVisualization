...
void Visualization::compute_vertex_velocities(std::string dir, bool skip_fix_lambdas, bool with_vertices)
{
    ...
    while(!lambda_range_generator.finished())
    {
        ...
        while(buffer_ptr->len() > 0) {
            ...
            const DevDatC vertices = hypercubes->get_vertices();
            const DevDatC vertex_velocities = hypercubes->get_vertex_velocities();
                          ...
                          const std::vector<double> shift_per_dim)
{
    ...
    // DevDatC saddle_coordinates = get_saddle_fix_points();
...
DevDatC Visualization::sample_around_saddle_point(const std::vector<double> coordinate, const std::vector<int> manifold_indices,
...
DevDatC Visualization::get_initial_values_to_eigenvector(const std::vector<double> saddle_point, const std::vector<cudaT> eigenvector, const std::vector<double> shift_per_dim)
{
