...
void ConditionalIntersectionObserver::initalize_side_counter(const DevDatC &coordinates)
{
    side_counter = compute_side_counter(coordinates, fixed_lambdas, indices_of_fixed_lambdas);
}


void ConditionalIntersectionObserver::operator() (const DevDatC &coordinates, cudaT t)
{
    if(monitor)
    {
...
void ConditionalIntersectionObserver::write_intersecting_separatrizes_to_file(const DevDatC &coordinates, const dev_vec_bool& intersections)
{
    ...
    {
        auto c = 0;
        DevDatC separatrizes(dim - fixed_lambdas.size(), coordinates.size() / coordinates.dim_size(), 0);
...
void ConditionalIntersectionObserver::write_vicinity_separatrizes_to_file(const DevDatC &coordinates, const dev_vec_bool& vicinity_to_fixed_dimensions)
{
...
dev_vec_bool ConditionalIntersectionObserver::check_for_vicinity_to_fixed_dimensions(const DevDatC &coordinates)
{
