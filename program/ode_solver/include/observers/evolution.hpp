...
typedef boost::numeric::odeint::runge_kutta_dopri5< DevDatC, cudaT,  DevDatC, cudaT > error_stepper;
typedef boost::numeric::odeint::runge_kutta4< DevDatC, cudaT,  DevDatC, cudaT > stepper;
...
public:
    ...
    EvolutionParameters(const json params_) : Parameters(params_),
                                              integration_type(get_value_by_key<std::string>("integration_type")),
                                              start_t(get_value_by_key<cudaT>("start_t")),
                                              delta_t(get_value_by_key<cudaT>("delta_t")),
                                              end_t(get_value_by_key<cudaT>("end_t")),
                                              number_of_observations(get_value_by_key<uint>("number_of_observations")),
                                              observe_every_nth_step(get_value_by_key<uint>("observe_every_nth_step")),
                                              step_size_type(mode_resolver.at(get_value_by_key<std::string>("step_size_type"))),
                                              results_dir(get_value_by_key<std::string>("results_dir")),
                                              max_steps_between_observations(get_value_by_key<uint>("max_steps_between_observations"))
    {}

    // Evaluate at equidistant time steps and fixed end time
    EvolutionParameters(const cudaT start_t_, const cudaT delta_t_, const cudaT end_t_,
                     const std::string step_size_type_="constant", const std::string results_dir_="",
                     const uint max_steps_between_observations_=500) :
            EvolutionParameters(json {{"integration_type", "equidistant_time_fixed_end"}, {"start_t", start_t_}, {"delta_t", delta_t_}, {"end_t", end_t_},
            {"number_of_observations", 0}, {"observe_every_nth_step", 0},
            {"step_size_type", step_size_type_}, {"results_dir", results_dir_},
            {"max_steps_between_observations", max_steps_between_observations_}})
    {}

    // Evaluate at equidistant time steps and a fixed number of observations
    EvolutionParameters(const cudaT start_t_, const cudaT delta_t_, const uint number_of_observations_,
                     const std::string step_size_type_="constant", const std::string results_dir_="",
                     const uint max_steps_between_observations_=500) :
            EvolutionParameters(json {{"integration_type", "equidistant_time_fixed_n"}, {"start_t", start_t_}, {"delta_t", delta_t_}, {"end_t", 0.0},
    ...
    EvolutionParameters(const uint observe_every_nth_step_, const cudaT start_t_, const cudaT delta_t_,
                     const cudaT end_t_, const std::string step_size_type_="constant", const std::string results_dir_="",
                     const uint max_steps_between_observations_=500) :
            EvolutionParameters(json {{"integration_type", "equidistant_integration_steps_fixed_end"}, {"start_t", start_t_}, {"delta_t", delta_t_}, {"end_t", end_t_},
    ...
    EvolutionParameters(const uint observe_every_nth_step_, const cudaT start_t_, const cudaT delta_t_,
            ...
            EvolutionParameters(json {{"integration_type", "equidistant_integration_steps_fixed_n"}, {"start_t", start_t_},
            ...
            {"results_dir", results_dir_}, {"max_steps_between_observations", max_steps_between_observations_}})
    {}
...
{
public:
    Evolution(const EvolutionParameters &ep_, FlowEquationsWrapper * flow_equations_) :
    ep(ep_), system(FlowEquationSystem(flow_equations_))
    {
        /* stepper
         * (store results_dir)
         * observer?
    ...
    void evolve(DevDatC &initial_coordinates, Observer &observer)
    {
    ...
    template<typename Observer>
    void evolve_on_condition(DevDatC &initial_coordinates, Observer &observer)
    {
...
private:
    ...
    const EvolutionParameters &ep;
    FlowEquationSystem system;

    template <typename StepperType, typename Observer>
    void evolve_(StepperType stepper, Integrator<StepperType, Observer> * integrator, DevDatC &initial_coordinates, cudaT start_t=0.0, cudaT end_t=1.0, cudaT delta_t=0.01)
    {
