...
public:
    ...
    void initalize_side_counter(const DevDatC &coordinates);

    void operator() (const DevDatC &coordinates, cudaT t) override;

    void write_intersecting_separatrizes_to_file(const DevDatC &coordinates, const dev_vec_bool& intersections);

    void write_vicinity_separatrizes_to_file(const DevDatC &coordinates, const dev_vec_bool& vicinity_to_fixed_dimensions);

    dev_vec_bool check_for_intersection(const DevDatBool& updated_side_counter);

    dev_vec_bool check_for_vicinity_to_fixed_dimensions(const DevDatC &coordinates);
