...
struct Integrator
{
    virtual void operator() (StepperType &stepper, FlowEquationSystem &system, DevDatC &initial_coordinates, cudaT &start_t, cudaT &end_t, cudaT &delta_t, Observer &observer) = 0;
};
...
{
    void operator() (StepperType &stepper, FlowEquationSystem &system, DevDatC &initial_coordinates, cudaT &start_t, cudaT &end_t, cudaT &delta_t, Observer observer) override
    {
