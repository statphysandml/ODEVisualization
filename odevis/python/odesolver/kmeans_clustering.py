...
class KMeansClustering:
    def __init__(self, maximum_expected_number_of_clusters: int, upper_bound_for_min_distance: float, maximum_number_of_iterations: int = 1000):
    ...
    def eval(self, coordinates, k: Optional[int] = None):
        if k is None:
            return Coordinates(self._kmeans_clustering.eval(coordinates.raw(), -1))
        else:
            return Coordinates(self._kmeans_clustering.eval(coordinates.raw(), k))
