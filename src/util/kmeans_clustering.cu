...
namespace odesolver {
    namespace util {
        ...
        KMeansClustering::KMeansClustering(const json params):
            Parameters(params),
        ...
        odesolver::DevDatC KMeansClustering::cluster_device_data(const odesolver::DevDatC &device_data) const
        {
            ...
            return cluster_device_data_fixed_k(device_data, appropriate_k);
        }

        odesolver::DevDatC KMeansClustering::cluster_device_data_fixed_k(const odesolver::DevDatC &device_data, const uint k) const
        {
            auto transposed_data = device_data.transpose_device_data();
            KMeans kmeans(k, transposed_data);
            kmeans.apply(maximum_number_of_iterations_);
