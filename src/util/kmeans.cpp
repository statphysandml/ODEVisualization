...
namespace odesolver {
    namespace util {
        ...
        void KMeans::initialize()
        {
            ...
            for(auto i = 0; i < k_; i++)
            {
                auto random_idx = random_data_point_generator_(g_gen);
                while(std::find(center_ids.begin(), center_ids.end(), random_idx) != center_ids.end())
                {
                    random_idx = random_data_point_generator_(g_gen);
                }
        ...
        void KMeans::apply(uint max_iterations)
        {
            initialize();
            auto c = 0;
        ...
        std::vector<uint> KMeans::expectation_step()
        {
        ...
        double KMeans::get_mean_distance_to_cluster_centers()
        {
            double mean = 0.0;
            for(auto i = 0; i < n_data_; i++) {
        ...
        std::vector<std::vector<double>> KMeans::get_centers()
        {
            return centers_;
        }
        ...
        std::vector<double> KMeans::compute_euclidean_distances(const std::vector<double> &dat, const std::vector<std::vector<double>> &centers)
        {
        ...
        double KMeans::compute_euclidean_distance(const std::vector<double> &dat1, const std::vector<double> &dat2) {
