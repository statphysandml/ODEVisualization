...
namespace odesolver {
    namespace modes {
        ...
        {
            for(auto i = 0; i < jacobian_elements_.size() / (dim * dim); i++)
            {
                jacobians_.push_back(MatXdMap(jacobian_elements_.data() + i * dim * dim, dim, dim));
            }
        }

        Jacobians Jacobians::from_vec_vec(std::vector<std::vector<double>> jacobian_elements)
        {
            return Jacobians(
                std::accumulate(jacobian_elements.begin(), jacobian_elements.end(), std::vector<double>{},
                [](auto &x, auto &y) {
        ...
        const std::vector<double> Jacobians::get_jacobian_elements() const
        {
        ...
        const Eigen::EigenSolver<Jacobians::MatXd>::EigenvectorsType Jacobians::get_eigenvector(const int i) const
        {
            return eigenvectors_[i];
        }

        const Eigen::EigenSolver<Jacobians::MatXd>::EigenvalueType Jacobians::get_eigenvalue(const int i) const
        {
            return eigenvalues_[i];
        }

        const std::vector<std::vector< std::vector<cudaT>>> Jacobians::get_real_parts_of_eigenvectors() const
        {
        ...
        const std::vector<std::vector<cudaT>> Jacobians::get_real_parts_of_eigenvalues() const
        {
        ...
        const std::vector<int> Jacobians::get_indices_with_saddle_point_characteristics() const
        {
            std::cout << eigenvalues_.size() << std::endl;
